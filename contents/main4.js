...
$('#motor_on').on('click', (e) => {
  on.callService(ROSLIB.ServiceRequest(), (result) => {
    if(result.success){
...
$('#motor_off').on('click', (e) => {
  off.callService(ROSLIB.ServiceRequest(), (result) => {
    if(result.success){
...
$('#touchmotion').on('click', (e) => {
  ...
  console.log('(vel_fw, vel_rot) = (' + vel_fw + ',' + vel_rot +')');
});
